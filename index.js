    ...
    let streams = (data.streams || [])
      ...
      .map(s => {
        ...
        return {
          ...
          name: "💥🟢SOUHAIL/RD🟢💥",
          title: `
🎬 ${extractCleanMovieTitle(title)}
💢 ${extractVideoRange(title)}    💾 ${formatSize(extractSize(title))}
📽️ ${extract(title, /(2160p|1080p|720p)/i)}    🎞️ ${extract(title, /(H\\.265|H\\.264|x265|x264)/i) || "H.264"}
🔊 ${extract(title, /(Atmos|DDP5\\.1|DD5\\.1|AC3|AAC)/i) || "Audio"}
🌍 EN / AR
🧲 ${extract(title, /(YTS|RARBG|TPB|ThePirateBay|1337x)/i) || "Torrent"}
⚡ RD Cached
          `.trim()
        };
      });
